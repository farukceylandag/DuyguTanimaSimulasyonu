...
function startWebcam() {
	...
	navigator.getMedia({
		...
		alert("Kameraya erişilemiyor.Kamera erişim ayarlarını kontrol edin.");
		isWebcamOn = 0;
	});
...
function captureSample(id, label) {
	if (isWebcamOn == 1) {
		...
		alert("Kameranızı açmak için 'Başlat' butonuna basın.")
	}
}
...
async function train() {
	...
	if(xs == null) {
		alert("Lütfen eğitimden önce bazı örnekler ekleyin.");
	} else {
		...
		if(!(batchSize > 0)) {
			alert("Lütfen küme boyutu için sıfır olmayan bir kesir seçin!");
		}
		...
		classifier.fit(xs, ys, {
			...
			callbacks: {
				onBatchEnd: async (batch, logs) => {
					lossTextEle.innerHTML = "Loss (Maliyet) : " + logs.loss.toFixed(5);
					await tf.nextFrame();
				}
			}
		});
